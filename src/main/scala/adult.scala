...
class Adult extends Actor {
  ...
  def receive = {
    ...
    case Adult.FeedKids => { doForAllChildren(Kid.Feed) }
    case Adult.PokeKids => { doForAllChildren(Kid.Poke) }
    case Adult.MeasureKids => { doForAllChildren(Kid.Size) }
    case Adult.Info(message) => { println("Adult got message: " + message) }
    case Adult.KillKids => {
